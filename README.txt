Data: https://drive.google.com/file/d/1B582y8_cPWxNuevpm3ZM-SZf_23HRUAQ/view?usp=share_link

File Navigation:
Code: Includes all the code files we have workes one
  Scrapped: Code that we did not use for the project
Deliverables: PDF of the deliverables submissions
charts and figures: includes all of our graphs and charts from this project
Final-Documents: The documents due for submission through canvas
visuals_logs_and_other_work: includes visuals and cluster outputs
